...
8. **Theming** — `--theme-50`…`--theme-950` for role/brand colors; semantic state uses consumer-provided `--status-*` tokens (`status-ok/warn/late/pending/info/danger` + `-contrast`), never hardcoded Tailwind palette values. `danger` is a true red for destructive actions, distinct from rose `pending`. Status tokens encode meaning, so they don't rotate per role/brand. **Chrome (boxes, borders, dividers, fills) MUST use `surface`/`ink`/`edge`/`status` tokens — NEVER raw `slate-*`/`gray-*`/`white`/`black` (they render as harsh dark lines on the host's light theme; the `Accordion` `slate-700` was exactly this bug). Light-theme contrast/hierarchy is the host's two-band surface model — see `jogi/docs/ref/theme-contrast.md`. Two cross-repo gotchas: (a) a novel arbitrary Tailwind alpha used ONLY here (e.g. `edge-subtle/12`) may not generate in the host and silently falls back — use alphas the host already uses (`/10`,`/15`,`/20`,`/25`); (b) after editing components, the host must `rm -rf .next` + restart dev, or Next keeps serving the old bundled build and your change appears to do nothing.**
