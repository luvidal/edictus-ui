...
```
@jogi/ui dirty (N files, M LOC). Commit + push? (y / n / w)
...
INSTALLED_SHA=$(grep -oE 'jogi-ui#[a-f0-9]{40}' ../jogi/package-lock.json | head -1 | grep -oE '[a-f0-9]{40}')
```
...
```bash
cd ../jogi
npm install @jogi/ui@github:luvidal/jogi-ui#$LOCAL_SHA
npx tsc --noEmit
```
