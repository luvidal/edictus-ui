...
const Accordion = ({ sections, forceExpanded = false, rememberOpen = true, storageKey, onChange }: AccordionProps) => {
    ...
    return (
        <div className='rounded-xl overflow-hidden border border-edge-subtle/20 bg-surface-1 shadow-token-sm flex-1 flex flex-col min-h-0'>
            {sections.map(section => {
                ...
                return (
                    <div key={section.id} className={`border-b border-edge-subtle/10 last:border-b-0 flex flex-col ${isExpanded ? 'flex-1 min-h-0' : ''}`}>
                        <button
                        ...
                        >
                            ...
                            <div className='flex-1 min-w-0 text-left flex items-baseline gap-1.5'>
                                ...
                                {section.subtitle && (
                                    <div className='text-[10px] text-ink-tertiary shrink-0'>{section.subtitle}</div>
                                )}
                            </div>
                            {!forceExpanded && (
                                <Icon
